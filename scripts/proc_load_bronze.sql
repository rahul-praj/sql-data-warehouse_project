...
BEGIN
    ...
    BEGIN TRY
        ...
        SET @endtime = GETDATE();
        PRINT '>> Load Duration: ' + CAST(DATEDIFF(second, @endtime, @start_time) AS NVARCHAR) + ' seconds';
        PRINT '>> -------------';
    ...
    -- SELECT COUNT(*) AS crm_cust_info FROM bronze.crm_cust_info;
    -- SELECT COUNT(*) AS crm_sales_details FROM bronze.crm_sales_details;
    -- SELECT COUNT(*) AS erp_cust_az12 FROM bronze.erp_cust_az12;
    -- SELECT COUNT(*) AS erp_loc_a101 FROM bronze.erp_loc_a101;
    -- SELECT COUNT(*) AS erp_px_cat_g1v2 FROM bronze.erp_px_cat_g1v2;
END;
GO
