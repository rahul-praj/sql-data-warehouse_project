...
BEGIN
    ...
    BEGIN TRY
        ...
        -- Load silver.erp_cust_az12 table
        SET @start_time = GETDATE();
        ...
        SELECT
            REPLACE(cid, '-', '') AS cid,
            CASE
                WHEN c.cntry_clean IS NULL OR c.cntry_clean = '' THEN 'n/a'
                WHEN c.cntry_clean = 'DE' THEN 'GERMANY'
                WHEN c.cntry_clean IN ('UNITED STATES', 'USA') THEN 'US'
                ELSE c.cntry_clean
            END AS cntry
        FROM bronze.erp_loc_a101 a
        CROSS APPLY (
            SELECT TRIM(REPLACE(REPLACE(a.cntry, CHAR(13), ''), CHAR(10), '')) AS cntry_clean
        ) c;
        SET @end_time = GETDATE();
        PRINT '>> Load Duration: ' + CAST(DATEDIFF(second, @start_time, @end_time) AS NVARCHAR) + ' seconds';
        PRINT '>> -------------';
